...
local CreateFXField = function (name, mode, x, y, strength, radius, visible, fxID)
	...
	local Temp = display.newCircle(0,0, radius)
	Temp:setFillColor  (V.gFXFieldColorR,V.gFXFieldColorG,V.gFXFieldColorB,.25)
	Temp.strokeWidth  = 2
	...
	if mode == 3 or mode == 4 then
		Temp  = display.newLine(-radius*.2,0, -radius*.2,-radius*.5)
		Temp:append(-radius*.5, -radius*.5)
		Temp:append(0, -radius)
		Temp:append( radius*.5, -radius*.5)
		Temp:append( radius*.2, -radius*.5)
		Temp:append( radius*.2, 0)
		Temp:append(-radius*.2, 0)
		Temp.strokeWidth  = 3
		Temp:setStrokeColor(V.gFXFieldColorR,V.gFXFieldColorG,V.gFXFieldColorB)
		V.FXFields[name]:insert(Temp)
...
local CreateEmitter = function (name, x, y, rotation, visible, loop, autoDestroy)
	...
	V.Emitters[name].x               = x
	V.Emitters[name].y               = y
	V.Emitters[name].anchorX         = .5
	V.Emitters[name].anchorY         = .5
	...
	-- EMITTER ARROW
	V.Emitters[name].Arrow = display.newLine(-8,0, 8,0)
	V.Emitters[name].Arrow:append   (0,-24, -8,0)
	V.Emitters[name].Arrow.strokeWidth = 3
	V.Emitters[name].Arrow:setStrokeColor (V.gEmitterColorR,V.gEmitterColorG,V.gEmitterColorB, 255)
...
----------------------------------------------------------------
local SetEmitterSound = function (name, SoundID, soundDelay, autoStop, SoundProperties)
	if V.Emitters[name] == nil then print ("!!! Particles.SetEmitterSound(): COULD NOT FIND EMITTER '"..name.."' OR MISSING SOUND ID."); return end
	...
	-- ATTACH NEW SOUND
		...
		V.Emitters[name].soundDelay  	= soundDelay ~= nil and soundDelay or 0
...
local CreateParticleType = function (name, Properties)
	...
	V.ParticleTypes[name].xReference            = .5
	V.ParticleTypes[name].yReference            = .5
	...
	V.ParticleTypes[name].emissionStep          = nil
	V.ParticleTypes[name].lastEmissionStep      = 0
	...
	V.ParticleTypes[name].DrawParent            = nil
...
local Update = function ()
	...
	if V.gLastTime    == 0 then V.gLastTime = system.getTimer() end

	local Emitter
	local now       = system.getTimer() - V.gLostTime
	local diffTime  = (now - V.gLastTime) * V.timeScale; V.gLastTime = now; if diffTime == 0 then diffTime = 1 end
	local i, j, v, n, eName, pName, dirAngle, ra, step
				...
				if PSettings.active == true and now > PSettings.startTime then
					...
					while PSettings.particlesToEmit >= 1.0 do
						...
						if PType.ImageSheet ~= nil then
							...
							V.Particles[slot].anchorX = PType.xReference
							V.Particles[slot].anchorY = PType.yReference
						-- TEXT PARTICLE
						elseif PType.text ~= nil then
							V.Particles[slot] = display.newText( PType.text, emitterX, emitterY, PType.font, PType.fontSize )
							V.Particles[slot]:setFillColor( PType.textColor[1],PType.textColor[2],PType.textColor[3]  )
							V.Particles[slot].anchorX = PType.xReference
							V.Particles[slot].anchorY = PType.yReference
						-- NORMAL IMAGE
						...
						elseif PType.emissionShape == 1 then
							if PType.emissionStep == nil then 
							     step = V.Rnd() * ra*2 
							else step = PType.lastEmissionStep
							     PType.lastEmissionStep = step + PType.emissionStep; if PType.lastEmissionStep > ra*2 then PType.lastEmissionStep = 0 end
							end
							local lineAngleL = V.PI-(emitterRot-90)/360*V.PI2
							Particle.x       = emitterX + (V.Sin(lineAngleL) * ra) - V.Sin(lineAngleL) * step
							Particle.y       = emitterY + (V.Cos(lineAngleL) * ra) - V.Cos(lineAngleL) * step

						elseif PType.emissionShape == 2 then
							if PType.emissionStep == nil then 
							     step = V.Rnd() * 360 
							else step = PType.lastEmissionStep
							     PType.lastEmissionStep = step + PType.emissionStep; if PType.lastEmissionStep > 360 then PType.lastEmissionStep = 0 end
							end
							local ringAngle = V.PI-(step)/360*V.PI2
						...
						elseif PType.emissionShape == 3 then
							...
							local discAngle  = V.PI-(step)/360*V.PI2
						...
						if PType.DrawParent == nil then 
							EmitterParent:insert(Particle) 
						-- ADD TO ANOTHER GROUP
						else 
							PType.DrawParent:insert(Particle) 
							local x,y = EmitterParent:localToContent(Particle.x, Particle.y)
							Particle.x, Particle.y = Particle.parent:contentToLocal(x,y)
						end 
	...
	for i = 1, maxParticles do
		...
		if V.Particles[i]  ~= nil then
			...
			for j,eName in ipairs(V.FXFieldIndex) do
				...
				px,py = P.x, P.y--P.parent:localToContent(px,py)
				fx,fy = Field.x, Field.y--Field.parent:localToContent(fx,fy)
					...
					-- INSIDE FIELD
						...
						if P.isPhysicsParticle == true then
							...
							if Field.mode == 0 and dst > 16 then
								...
								P:applyForce( (fx-px)*power, (fy-py)*power , px,py )
							-- REJECTOR
							elseif Field.mode == 1 then
								power = (1.0-(dst/Field.radius)) * Field.strength
								P:applyForce( (px-fx)*power, (py-fy)*power, px,py )
							-- PARTICLE KILLER
							...
							elseif Field.mode == 3 then
								...
								P:applyForce( V.Sin(dirAngle)*power, V.Cos(dirAngle)*power, px,py )
							-- HARD REDIRECTOR
							elseif Field.mode == 4 then
								dirAngle = V.PI-Field.rotation/360*V.PI2
								P:setLinearVelocity( V.Sin(dirAngle)*Field.strength, V.Cos(dirAngle)*Field.strength, px,py )
							end
							...
							if Field.mode == 0 and dst > 16 then
								...
								P.xSpeed = P.xSpeed + (fx-px) * power;
								P.ySpeed = P.ySpeed + (fy-py) * power;
							-- REJECTOR
							elseif Field.mode == 1 then
								local ang= ( V.Atan2( (fy-py), (fx-px) ) * (180 / V.PI) ) - 90 
...
----------------------------------------------------------------
V.CountEmitters = function (activeOnly)
	local i
	local num = 0
	if activeOnly ~= true then return #V.EmitterIndex else
		for i,eName in ipairs(V.EmitterIndex) do
			if V.Emitters[eName].active == true then num = num + 1 end
		end
		return num
	end
end
...
local SetEmitterColor = function (r,g,b)
	...
	for i,eName in ipairs(V.EmitterIndex) do
		V.Emitters[eName]:setStrokeColor (V.gEmitterColorR,V.gEmitterColorG,V.gEmitterColorB)
	end
end
